export type CrownAndCaliberSearchCard = {
...
export function parseCrownAndCaliberSearch(doc: Document): CrownAndCaliberSearchCard[] {
  const cards = Array.from(doc.querySelectorAll<HTMLElement>(".product-card"));
  return cards
    .map((el) => {
      const titleText = el.querySelector(".product-card__title")?.textContent?.trim() ?? "";
    ...
    .filter((c) => c !== null) as CrownAndCaliberSearchCard[];
}
