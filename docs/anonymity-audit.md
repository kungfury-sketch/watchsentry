...
|---|---|---|---|
| ~~Operator workspace path `C:\omerprojects\watchsentry\` appears in committed plan, progress log, and audit doc~~ | ~~Session 0 plan was written with literal local paths; pre-existing finding~~ | **RESOLVED 2026-05-20 (commit `7940fa7`)** — 174 substitutions across `docs/plans/` and `progress/session-log.md` replacing the operator path with `<repo>` / `<workspace>` placeholders. Em-dashes and UTF-8 preserved. Audit doc itself retains the descriptive references to the issue (intentional). | _resolved_ |
| ~~Placeholder extension icons (1×1 transparent PNGs) in `extension/icons/`~~ | ~~Session 4 manifest needed icons to build; brand-design phase deferred~~ | **RESOLVED 2026-05-22 (commit `20a6f66`)** — Phosphor ShieldCheck (MIT, no attribution) rendered at 16/48/128 via sharp; white stroke on #1F6FEB accent blue. Sizes: 273B / 709B / 1755B. | _resolved_ |
